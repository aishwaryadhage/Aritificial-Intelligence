...
#
# Your names and user ids: Aishwaray Dhage(adhage), Ninaad Joshi(ninjoshi), Siddharth Pathak(sidpath)
#
...
# and after the current word i.e the the POS tag of word2 depends on what the word1 is and word3 is.
# HMM handles this nicely, as it takes into consideration the transistion probability between words based on the HMM model.
# Each hidden node in the HMM is a POS tag, and observed node is a word.
...
# Third step is to calculate posterior marginals of each hidden state i.e  P(Wk|S)=P(Sk+1:n|Wk)*P(Wk|S1:k) where n is the number of states and k is the current state.
# In this algorithm we have considered punishing factor as 10**-8  for emission probability and transition probability if that tag is not present in the dictionary.
# We observed change in accuracy as we change the punishing factor,the most appropriate one we found for our algorithm is 10**-8.
# We have also observed from the percentage accuracy that VE gives one percent better results than Viterbi in some cases.
...
# Using these values, Viterbi calculates MAP, and gives sequence of POS tags for the input sentence.
